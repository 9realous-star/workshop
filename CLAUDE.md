...
| `activity.html` | Shared per-activity board page (`?id=a1`-`a4`): anyone can post a text+photo entry, admin-only "delete all" for that activity. Talks to `/api/refresh-activities`. For `a4` (단합 게임) only, an extra "게시판 / 조 편성" tab toggle shows static group assignments (hardcoded `TEAMS` array, no API). |
...
| `quiz.html` | Team trivia quiz, 10 hardcoded questions (OX + short-answer, mix of work/personal trivia) navigated one at a time with a reveal-on-click answer; no scoring, no persistence. Some answers are still placeholder text pending real answers from the team lead. |
...
|---|---|---|---|---|
| `api/praises.js` | GET/POST/PATCH/DELETE | `?mode=roast`→`ws_praises_v4` (default), `?mode=respect`→`ws_praises_respect_v1` | `ADMIN_PW='0512'` hardcoded, checked via `?pw=` for bulk DELETE | Full updated array (for that mode's key) on every mutation; `{error}` + 400/403/404/405 on failure |
